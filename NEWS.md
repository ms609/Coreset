...
- `Gonzalez()`: deterministic farthest-first selection from a distance matrix,
  ...
  seeds — `"centroid"`, `"peripheral"`, and `n_random` (3) reproducible
  `"random_furthest"` starts drawn from a fixed internal seed — and keeps the
  best by `TkScore()`. Costlier O(*N*²) anchors (`"diameter"`, `"anti_medoid"`,
