...
- Ensemble functions now attach a `score = NA_real_` attribute on the trivial
  all-points early return (when \code{m >= N}).
- Integer iteration counters in the C++ kernels changed from \code{int} to
  \code{long long} to avoid signed-integer overflow UB at extreme \code{maxIter}
  values.
- Test suite: improved coverage of path relinking (strict improvement), DropAdd
  \code{secondary} attribute formula, \code{ExactMaxMin} budget-expiry branch,
