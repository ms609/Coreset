...
- `Gonzalez()`: deterministic farthest-first selection from a distance matrix,
  ...
  embedding. A `seed` argument chooses a peripheral seeding strategy
  (`"diameter"`, `"antimedoid"`, `"medoid"`, `"rowsum"`, `"rownorm"`) with a
  robust `"ensemble"` default that keeps the best by `TkScore()`.
