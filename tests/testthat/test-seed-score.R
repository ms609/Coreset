...
test_that("centroid seed is the farthest point from the coordinate mean", {
  ...
  expect_error(FarFirst(dat$d, 5L, method = "centroid"), "coordinates")
})
...
test_that("Gonzalez method='first' uses index 1 as anchor (both paths)", {
  dat <- MakeData()
...
test_that(".GonzEnsemble non-first anchor wins when it is the better one", {
  ...
  for (s in seq_len(50L)) {
    ...
    n   <- 6L
    tkD  <- MinDist(d, FarFirst(d, n, method = "diameter"))
    tkAm <- MinDist(d, FarFirst(d, n, method = "anti_medoid"))
