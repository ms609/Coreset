...
test_that("DropAdd smoke: 20 pts in 5-D, m=5", {
  ...
  d <- dist(pts)
  res <- DropAdd(d, m = 5L, time_budget_s = 1)
...
test_that("DropAdd construction-only result matches its MaxMin score", {
  ...
  res <- DropAdd(dmat, m = 6L, max_iter = 0L)
...
test_that("DropAdd never worsens the constructive solution", {
  ...
  cons <- DropAdd(dmat, m = 10L, max_iter = 0L)
  full <- DropAdd(dmat, m = 10L, max_no_improve = 2000L)
...
test_that("DropAdd reaches the Geo 100 1 m=10 optimum (89.37)", {
  ...
  res <- DropAdd(dmat, m = 10L, time_budget_s = 10, max_no_improve = 100000000L)
  best_known <- 89.37
...
test_that("DropAdd .verify=TRUE passes silently on a real instance", {
  ...
  expect_silent(
    DropAdd(dmat, m = 10L, time_budget_s = 30, max_iter = 100L,
              .verify = TRUE)
  )
})
...
test_that("DropAdd FIFO drops each initial point once in first m iterations", {
  ...
  trace_env <- new.env()
  res <- DropAdd(dmat, m = m, time_budget_s = 60, max_iter = 2L * m,
                   .trace = trace_env)
...
# ---------------------------------------------------------------------------
test_that("DropAdd respects time_budget_s within reasonable slack", {
  set.seed(99)
  pts <- matrix(runif(200 * 5), ncol = 5)
  dmat <- as.matrix(dist(pts))
  t0 <- Sys.time()
  # Disable stagnation so the wall-clock ceiling is the binding criterion.
  res <- DropAdd(dmat, m = 20L, time_budget_s = 1, max_no_improve = 100000000L)
...
test_that("DropAdd seed parameter and input validation", {
  ...
  # seed path
  r1 <- DropAdd(dmat, m = 4L, max_iter = 0L, seed = 1L)
  r2 <- DropAdd(dmat, m = 4L, max_iter = 0L, seed = 1L)
  ...
  expect_error(DropAdd(dmat, m = 4L, max_no_improve = 0L),  "max_no_improve")
  # max_iter validation
  expect_error(DropAdd(dmat, m = 4L, max_iter = -1L),  "max_iter")
  # time_budget_s validation
  expect_error(DropAdd(dmat, m = 4L, time_budget_s = 0),   "time_budget_s")
  expect_error(DropAdd(dmat, m = 4L, time_budget_s = NA_real_), "time_budget_s")
})

test_that("DropAdd progress = TRUE fires the cli hooks", {
  dmat <- as.matrix(dist(matrix(rnorm(15 * 2), ncol = 2)))
  expect_no_error(DropAdd(dmat, m = 3L, max_iter = 2L, progress = TRUE))
})
...
test_that("DropAdd construction tie-breaking and caseA update (.verify)", {
  ...
  res <- DropAdd(dmat, m = 3L, max_iter = 4L, .verify = TRUE)
...
test_that("DropAdd m=2 else-branch and ADD tie-breaking (.verify)", {
  ...
  res <- DropAdd(dmat, m = 2L, max_iter = 4L, .verify = TRUE)
...
test_that("DropAdd caseA ADD update fires in main loop (.verify)", {
  ...
  dmat   <- as.matrix(dist(pts_sq))
  expect_no_error(
    DropAdd(dmat, m = 3L, max_iter = 3L, .verify = TRUE)
  )
})

test_that("DropAdd effective_max = 0 when m == n (.verify)", {
...
test_that("DropAdd .trace + .verify together cover init and loop writes", {
  ...
  te   <- new.env()
  res  <- DropAdd(dmat, m = 4L, max_iter = 5L, .verify = TRUE, .trace = te)
...
test_that("DropAdd C++ construction covers sum_dist tie-break (lines 82-85)", {
  ...
  res <- DropAdd(d, m = 4L, max_iter = 0L)
...
test_that("DropAdd C++ m=2 covers DROP else-branch (lines 214-215)", {
  ...
  res <- DropAdd(d_rh, m = 2L, max_iter = 4L)
  expect_length(res$indices, 2L)
})
...
test_that("DropAdd C++ main-loop ADD covers tie-break (255-258) and equality (277)", {
  ...
  res <- DropAdd(d7, m = 4L, max_iter = 1L)
  expect_length(res$indices, 4L)
...
test_that("DropAdd R reference loop and C++ port are bit-identical", {
  ...
  for (max_iter in c(0L, 1L, 5L, 50L, 200L)) {
    out_R <- DropAdd(dmat, m = 8L, max_iter = max_iter, .verify = TRUE)
    out_C <- DropAdd(dmat, m = 8L, max_iter = max_iter)
  ...
  for (mni in c(5L, 25L, 100L)) {
    out_R <- DropAdd(dmat, m = 8L, max_no_improve = mni, .verify = TRUE)
    out_C <- DropAdd(dmat, m = 8L, max_no_improve = mni)
    expect_identical(out_R$indices,   out_C$indices)
    expect_identical(out_R$objective, out_C$objective)
    expect_identical(out_R$secondary, out_C$secondary)
    expect_identical(out_R$iters,     out_C$iters)
