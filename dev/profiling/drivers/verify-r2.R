...
# T-005 DropAdd seed cache-fix, T-006 GraspPR incremental swap). All three are
...
# FarFirst (points + matrix), DropAdd (matrix + points), GraspPR.
for (ds in 1:4) {
    ...
    for (dim in c(2L, 5L)) {
      ...
      for (m in c(5L, 20L, as.integer(N / 3))) {
        for (es in c(3L, 5L)) for (al in c(0.5, 0.8)) {
          g <- GraspPR(d, m, plateau = 10L, eliteSize = es, alpha = al, seed = 7L)
...
# T-006 GraspPR large m.
set.seed(12)
for (cfg in list(c(200,50), c(200,100))) {
  N <- cfg[1]; m <- cfg[2]
  pts <- matrix(rnorm(N * 2L), ncol = 2L); d <- as.matrix(dist(pts))
  timing[[paste0("grasp_n", N, "_m", m)]] <- list(
    what = "GraspPR", N = N, m = m,
    ms = ms(function() GraspPR(d, m, plateau = 15L, eliteSize = 5L, seed = 1L), it = 5L))
}
