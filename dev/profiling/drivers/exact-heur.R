...
for (cs in c("tc22_penguins", "tc11_ionosphere")) {
  ...
  opt <- Coreset::ExactMaxMin(d, k, maxSeconds = 600)$objective
  da512  <- sc(d, Coreset::DropAdd(d = d, m = k, plateau = 512L))
  da5000 <- sc(d, Coreset::DropAdd(d = d, m = k, plateau = 5000L))
  gp <- sc(d, Coreset::Grasp(d, k, plateau = 50L, seed = 1L))
