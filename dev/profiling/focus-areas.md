...
| 4 | ExactMaxMin node-packing | `R/exact.R`, `src/exact_reduce.cpp` | pure-R orchestration of highs MILP probes; per-probe combinatorial reduction in C++ | matrix only (highs) | m ∈ {2,4,6,10} (exact regime) | 2026-08-14 | OPTIMISED (round 5: sparse-A + Grasp warm-start gallop, 19.6× grid, T-008; round 11: complement-graph (k−1)-core peel + colouring bound + clique-strengthened per-component IPs — scores bit-identical 56/56, grid ~2×, spam n=4601 proven 2.4 s vs 201 s/8.9 GB, satellite n=6435 k=4 proven 5.6 s. See log.md rounds 5, 11) |
