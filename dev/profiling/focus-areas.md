...
| 3 | Grasp refinement | `src/grasp.cpp`, `R/grasp.R` | extended-improvement local search scans (n−m) candidates × O(m²) sub-matrix; PR over elite pairs | matrix only | m ∈ {small, large} | 2026-08-13 | AT-LIMIT serial (rounds 3–5, T-006/7/14/16/17/18/19 cumulative; round 5 +1.45× serial, then T-021 parallel — nCores-invariant results, 5.9× at 8 threads). Serial floor: tie-break-mandated O(n) screen, construction's O(n)/step sequential update, tie-arm memo. Remaining axis: cores + Hamilton. See log.md round 5 |
