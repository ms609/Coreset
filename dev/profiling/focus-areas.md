...
|---|------|-------|------------------|-------|-----------|---------------|--------|
| 1 | FarFirst greedy + ensemble | `src/maximin.cpp`, `src/maximin_points.cpp`, `R/seed.R`, `R/farfirst.R` | O(N·n·dim) greedy pass run 3× per default ensemble; O(N²·dim) seeding anchors | matrix, points, column-oracle | n/N ∈ {0.01 … 0.5} | 2026-08-13 | AT-LIMIT serial (rounds 1+7: free-T_k, squared space, column reorder, argmax fold + fused sweep — points dim=2 ~1.45×); round 7 adds mc.cores kernels, results invariant to thread count (N=1e5 pass 2.5–4.5×, O(N²) anchors 5.3× at 8T). Matrix path bandwidth-bound. See log.md round 7 |
