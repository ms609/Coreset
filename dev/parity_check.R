...
for (n in c(2L, 5L, 10L)) {
  ...
  chk(sprintf("ensemble   points n=%d", n),
      strip(MaxMin::Gonzalez(n = n, points = pts, seed = "ensemble")),
      strip(FurthestPoint::WideSampleGonzEnsemble(n = n, points = pts)))
}

# GonzalezColumn oracle
colFn <- function(i) dm[, i]
chk("GonzalezColumn first=1",
    MaxMin::GonzalezColumn(colFn, nrow(dm), 8L, first = 1L),
    FurthestPoint::GonzalezColumn(colFn, nrow(dm), 8L, first = 1L))
chk("GonzalezColumn peripheral seed",
    MaxMin::GonzalezColumn(colFn, nrow(dm), 8L),
...
# PolishSelection
s0 <- MaxMin::Gonzalez(dm, 8L, first = 1L)
chk("PolishSelection",
