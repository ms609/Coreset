...
| 9 | MaxMean (max-mean dispersion / RLTS) | `R/maxmean.R`, `src/maxmean.cpp`, `R/score.R` (`MeanDist`) | sonnet | Objective math: is `f(S)=sum_{i<j}d_ij/\|S\|` (divide by \|S\|, not pair count) maintained exactly by the incremental `P[i]` array across add/remove flips? Add/remove deltas (Eq. 9) and `sum_pairs`/`p[u]` updates — stale entries, sign errors, `p[u]` self-handling? `\|S\|>=2` floor on removal; `m==n` (no add) and 2-element corners. RL layer: Q-update (Eq. 4), epsilon-greedy `Sa` compaction (off-by-one, duplicate/skip actions), reward-matrix sign (`±cur_r`), termination Eq. 7. Tabu: tenure sawtooth, aspiration admissibility (union-max), per-restart reset. RNGScope present? `i*n` overflow at large n? Signed/negative distances. NA/Inf rejection. `MeanDist` vs C++ objective agreement. |
