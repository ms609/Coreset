...
Benchmark(Gonzalez(d2000, 20L))                       # seed = "ensemble"
...
# Distance-column oracle path (as used by TreeSearch on tree sets): the column
# is recomputed from coordinates on the fly via PointColumn(), never a matrix.
colFn <- function(i) PointColumn(pts5000, i)
Benchmark(GonzalezColumn(colFn, 5000L, 30L))
