...
By default `FarFirst()` runs eight `"random_furthest"` starts, each beginning
from a distinct point that is furthest from a randomly selected 'pivot' point,
and returns whichever pass produced the highest T~k~.
...
ffPick <- FarFirst(6L, eurodist, nseeds = 2L)
```
