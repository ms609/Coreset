...
| `Grasp()` | Highest | Moderate | Yes (`seed =`) |
...
matrix.
`FarFirst()` calls it $n$ times, so the full N×N matrix is never built.
...
`Grasp()` is stochastic and usually edges out `DropAdd()` on T~k~, but
