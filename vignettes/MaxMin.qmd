...
The Max-Mean Dispersion Problem rewards the average separation; it maximizes
the sum of pairwise distances divided by the number of selected elements,
which is chosen by the algorithm.

`MaxMean()` implements reinforcement-learning tabu search (RLTS) 
...
elements that would dilute it. We thus draw a symmetric matrix of signed
dissimilarities:
...
`MaxMean()` chooses the subset size for us, retaining only the elements that
