...
The default `seed = "ensemble"` runs four peripheral seeding strategies and
returns whichever produced the highest T~k~:

```{r}
#| label: gonzalez-ensemble
idx_ens <- Gonzalez(eurodist, n = 6L)   # ensemble is the default

# Which seeding strategy won?
attr(idx_ens, "winning_strategy")
```

You can also request a specific seeding strategy, or pass `first` to fix the
starting index manually:
...
When your items live in a space with no coordinate embedding — for example,
phylogenetic trees with patristic distances — `GonzalezColumn()` accepts an
*on-demand distance-column oracle* and avoids materialising the full matrix
entirely.
