...
based on one of three complementary objectives:
...
Unlike the other objectives, the subset size is not fixed in advance but is
itself chosen to maximize this equity-based measure of dispersion, and the
distances may be negative.
This objective arises in applications such as social-network analysis,
web-page ranking and pollution control [@Dieudonne2020].
...
| `MaxMean()` | max-mean | High (RLTS) | Time-budgeted | Yes (`set.seed()`) |
...
The max-min objective above is governed entirely by the *closest* pair in the
selection; a single tight pair caps the score however well-separated the
remaining elements are.
The **Max-Mean Dispersion Problem** instead rewards the *average* separation,
maximizing the sum of pairwise distances divided by the number of selected
elements [@Dieudonne2020].
Two features distinguish it from the max-min solvers: the subset size is chosen
by the algorithm rather than fixed in advance, and the distances may be
negative (so that a pair of elements can actively detract from a selection).

`MaxMean()` implements the reinforcement-learning tabu search (RLTS) of
@Dieudonne2020: each restart constructs a candidate selection — randomly at
first, then guided by a *Q*-learning memory of which elements proved valuable —
and refines it with a one-flip tabu search that adds or removes a single
element at a time.
Because the search restarts until its time budget expires, a longer
`maxSeconds` can only improve the result; set a seed for reproducibility.

The objective is most interesting when distances are signed: a pair separated
by a *negative* distance drags the mean down, so the optimal selection excludes
elements that would dilute it.
Following the problem's standard benchmarks [@Dieudonne2020], we draw a
symmetric matrix of signed dissimilarities:
...
`MaxMean()` chose the subset size for us, retaining only the elements that
raise the average separation.
`MeanDist()` scores any index set under the same objective — the sum of pairwise
distances divided by the number of selected elements — so a hand-picked
...
With strictly non-negative distances (as in `eurodist`) the mean rises
monotonically as elements are added, so the objective simply selects the entire
pool; the signed case above is where the size trade-off bites.

The *Q*-learning layer occupies memory of the same order as the distance matrix
($O(N^2)$); for very large instances it can be disabled with `useRL = FALSE`,
falling back to random restarts.
...
For **average dispersion** (maximize mean separation; subset size free):
