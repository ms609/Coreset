...
would be too expensive (e.g. large phylogenetic trees, where each patristic
distance requires a tree traversal), `GonzalezColumn()` accepts a user-supplied
function that returns one column of the distance matrix at a time.
Gonzalez needs only *n* such columns, so the full N×N matrix is never built.
...
| Arbitrary metric with no coordinate embedding | `GonzalezColumn()` |
