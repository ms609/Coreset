...
based on one of five complementary objectives:
...
The **Max-Sum Diversity Problem** (the "maximum diversity problem") instead
selects $k$ elements that maximize the *total* pairwise distance they contain.
Unlike the MMDP, whose objective is set by a single closest pair, every pair
in the selection contributes to the max-sum score, so the optimum tends to
favour elements spread across the whole extent of the pool rather than
elements that merely avoid a single close neighbour.

The **Max-Mean Dispersion Problem** instead maximizes the *average* distance
between selected elements: the sum of pairwise distances divided by the number
of elements selected, which is not specified in advance.
...
The **maximum-entropy (maxdet) sampling** objective selects $k$ elements that
maximize the log-determinant of a similarity kernel built from the distances
between them -- the spanned volume of the selection. A redundant element lies
in the span of those already chosen and adds no volume, so this objective is
"density-blind": it never rewards clusters of near-duplicate elements, without
requiring the explicit pairwise-distance thresholds that the other objectives
rely on.

An approximate selection that satisfies the max-min and *k*-centre objectives
within a factor of two of their respective optima can be attained by a greedy
farthest-first algorithm [@Gonzalez1985], though the exact optima typically
differ; dispersion spreads to the extremes, whereas covering gravitates into
the interior.
...
| `MaxEntropy()` | maxdet (max log-det) | High / Optimal for small `k` | Fast | No |
...
## Max-sum diversity (maximum diversity)

The **Max-Sum Diversity Problem** is the max-sum counterpart of the MMDP: it
selects the $k$-subset maximising the *total* pairwise distance it contains,
rather than the single minimum pairwise distance.
`ExactMaxSum()` solves it exactly via per-node integer-program linearisation
[@Kuo1993], floored by a multi-start 1-swap local search that both warms the
lower bound and is returned when the integer program cannot prove optimality
within `maxSeconds`.
Like `ExactMaxMin()`, it uses the **highs** solver and is a small-instance,
NP-hard tool.
...
Because every pair in the selection, not just the closest, contributes to the
max-sum objective, `ExactMaxSum()` and `ExactMaxMin()` can select different
subsets from the same instance.

```{r}
#| label: maxsum-vs-maxmin
#| eval: !expr requireNamespace("highs", quietly = TRUE)
smPick   # optimal for total pairwise distance
exPick   # optimal for the minimum pairwise distance
```
...
The Max-Mean Dispersion Problem maximizes the sum of pairwise distances divided
by the number of selected elements, which is chosen by the algorithm.
...
`MaxEntropy()` selects the $k$-subset that maximizes the log-determinant of
its kernel block, $\log\det K_S$ -- the maximum-entropy sampling criterion
[@Shewry1987], equivalently the maximum-a-posteriori mode of a determinantal
point process [@Kulesza2012].
A radial-basis kernel is built from the distance matrix and repaired to be
positive semi-definite where needed, since an arbitrary distance is not
guaranteed to be of negative type.
Because a redundant point lies in the span of those already chosen and adds
no volume, it is never selected: the objective is exactly density-blind,
unlike the distance-based objectives above.
...
enumeration when $\binom{n}{k}$ does not exceed `maxCombos` -- as here, where
the search is certified optimal.
`negMass` reports the fraction of spectral mass removed by the
positive-semidefinite repair, so a caller can see when that approximation is
doing real work:
